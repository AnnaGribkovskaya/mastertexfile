...
In the end of 19th century physics has some unsolved problems that can't be tackled using already developed theories and methods. This had led to a  significantly different theory that has a number of essential distinctions  from that developed before. This new theory was named quantum mechanics. In the classical (or Newtonian mechanics) there was a theoretical possibility to obtain a complete knowledge of the system under consideration. In quantum mechanics this is not possible. Both for some particular time moment and for all future or previous times. \\
...
\end{theorem}
We do not provide proofs for this Theorems $\ref{theorem:eigvalHerm}$, $\ref{theorem:eigvecHerm}$ and $\ref{theorem:orhtonormalbasis}$. For more detailed description please refer to \cite{aulettaQuantumMechanics2009}.
\begin{post}
...
!!!!!\\
We have already presented the Hamiltinain of such system in Section $\ref{sec:manybody}$ equations $\ref{H1H2}$. The unperturebed Hamiltonian eigenvalue can be presented as :
\begin{eqnarray}
