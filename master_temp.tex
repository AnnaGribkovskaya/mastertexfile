...
\begin{align}
\braket{D_{\textbf{n}|}| 1-\tau (\hat{H}-E)|\Psi_{CC}},\\
...
\end{align}
where we consider determinants $D_{\textbf{n}}$ and $D_{\textbf{m}}$ to be connected,so that  $H_\textbf{nm} \neq 0 $. This equation is similar to equation ($\ref{eq:CIprojected}$). Once aging following the same procedure as for FCIQMC we are aiming to create a population of walkers in order to discretize the amplitudes. In case of CCQMC walkers are called "excips" and the corresponding amplitudes are called "excitors" \footnote{text}.
